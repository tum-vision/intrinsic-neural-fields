"""
Adapted from
https://github.com/fbxiang/NeuTex/blob/master/models/atlasnet/networks.py
"""
import torch
...
# Transforms uv-map points into 3D points on the manifold
class Atlasnet(nn.Module):
    def __init__(
        ...
        self.decoder = nn.ModuleList(
            [
                Mapping2Dto3D(self.input_point_dim, activation=activation)
                for _ in range(0, num_primitives)
            ]
        )
    ...
    def sample_uvs_and_transform_to_3d(self, regular_point_count=None, device="cuda"):
        """
        ...
        if regular_point_count is None:
            input_points = [
                self.template[i].get_random_points(
                    self.num_points_per_primitive, device,
                )  # num_points_per_primitive x in_dim
                for i in range(self.num_primitives)
            ]
        else:
            input_points = [
                self.template[i].get_regular_points(
                    regular_point_count, device
                )
                for i in range(self.num_primitives)
            ]
        ...
        points2d = [
            self.decoder[i](
                input_points[i].unsqueeze(0)  # 1 x num_points_per_primitive x in_dim
            ).unsqueeze(1)  # 1 x num_points_per_primitive x 3
            # self.decoder[i](
        ...
        output_points = torch.cat(points2d, dim=0)  # nb_prim x num_point x 3
        input_points = torch.stack(input_points, dim=0)  # nb_prim x num_points x in_dim

        return input_points, output_points.contiguous()

    # Mapping from UV points to 3D
    # def map_uv_to_3d(self, latent_vector, uvs):
